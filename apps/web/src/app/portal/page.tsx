      ...
      <div style={{ padding: '18px 22px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 48, height: 48, borderRadius: 14, background: `linear-gradient(135deg, ${p?.color || '#333'}cc, ${p?.color || '#333'}88)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 900, fontSize: 20, flexShrink: 0, boxShadow: `0 0 16px ${p?.color || '#333'}40` }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 9px', borderRadius: 980, background: isExpired ? 'rgba(248,113,113,0.1)' : 'rgba(74,222,128,0.1)', border: `1px solid ${isExpired ? 'rgba(248,113,113,0.2)' : 'rgba(74,222,128,0.2)'}`, fontSize: 10, fontWeight: 700, color: isExpired ? '#f87171' : '#4ade80', letterSpacing: '0.06em' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
            <button style={{ height: 34, padding: '0 14px', borderRadius: 9, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#a1a1a6', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>Support</button>
...
export default function ClientPortal() {
  ...
  // Sync display info from Firebase user
  useEffect(() => {
    if (user) {
      setDisplayName(user.displayName || userData?.name || 'User')
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: '80px 24px 80px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
          style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40, flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            {/* Avatar with edit button */}
            <div style={{ position: 'relative', flexShrink: 0 }}>
              <div style={{ width: 64, height: 64, borderRadius: '50%', background: photoURL ? 'transparent' : 'rgba(212,175,55,0.15)', border: '2px solid rgba(212,175,55,0.3)', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {photoURL ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={photoURL} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <span style={{ fontSize: 22, fontWeight: 800, color: '#D4AF37' }}>{cardName.charAt(0).toUpperCase()}</span>
                )}
              </div>
            ...
            <div>
              <p style={{ fontSize: 10, fontWeight: 700, color: '#D4AF37', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 5 }}>My Account</p>
              <h1 style={{ fontSize: 'clamp(22px, 4vw, 36px)', fontWeight: 800, color: '#f5f5f7', letterSpacing: '-0.04em', lineHeight: 1.1 }}>
          ...
          <div style={{ display: 'flex', gap: 9, alignItems: 'center' }}>
            ...
            <button onClick={() => setShowEditProfile(true)}
              style={{ height: 38, padding: '0 16px', borderRadius: 9, background: 'rgba(212,175,55,0.07)', border: '1px solid rgba(212,175,55,0.2)', color: '#D4AF37', fontSize: 12, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              <Edit3 size={13} />Edit Profile
            </button>
            <button onClick={async () => { await signOut(); router.push('/') }}
              style={{ height: 38, padding: '0 16px', borderRadius: 9, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', color: '#666', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>
              Sign Out
            </button>
          </div>
        </motion.div>
