  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <p style={{ textAlign: 'center', fontSize: 11, color: '#555', marginBottom: 20 }}>Tap to change photo · Max 2MB</p>
...
function Skeleton() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {[1, 2, 3].map(i => <div key={i} style={{ height: 80, borderRadius: 18, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.05)' }} />)}
    </div>
  )
}
...
export default function ClientPortal() {
  ...
  useEffect(() => { if (user) { setDisplayName(user.displayName || userData?.name || 'User'); setPhotoURL(user.photoURL || '') } }, [user, userData])
  useEffect(() => {
  ...
  const isClient = ['bronze','silver','gold','elite-gold'].includes(cardTier)
  ...
  return (
    <div style={{ background: '#000', minHeight: '100vh' }}>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: isMobile ? '64px 16px 48px' : '80px 24px 80px' }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={mc}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16 }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p style={{ fontSize: 10, fontWeight: 700, color: '#D4AF37', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 3 }}>My Account</p>
              <h1 style={{ fontSize: isMobile ? 20 : 'clamp(22px, 4vw, 36px)', fontWeight: 800, color: '#f5f5f7', letterSpacing: '-0.04em', lineHeight: 1.15 }}>
                Welcome back, <span style={{ color: '#D4AF37' }}>{cardName.split(' ')[0]}</span>
              </h1>
              <p style={{ fontSize: 11, color: '#555', marginTop: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {activeSubs.length} active · {email}
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexDirection: isMobile ? 'row' : 'row', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
            ...
            <button onClick={() => setShowEditProfile(true)}
              style={{ height: 40, padding: '0 16px', borderRadius: 10, background: 'rgba(212,175,55,0.07)', border: '1px solid rgba(212,175,55,0.2)', color: '#D4AF37', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, flex: isMobile ? 1 : 'none' }}>
              <Edit3 size={13} />Edit Profile
            </button>
            <button onClick={async () => { await signOut(); router.push('/') }}
              style={{ height: 40, padding: '0 16px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', color: '#666', fontSize: 13, fontWeight: 500, cursor: 'pointer', flex: isMobile ? 1 : 'none' }}>
              Sign Out
            </button>
          </div>
        </motion.div>

        <hr style={{ border: 'none', height: 1, background: 'linear-gradient(to right, transparent, rgba(212,175,55,0.15), transparent)', marginBottom: isMobile ? 20 : 28 }} />

        {/* ── Main grid ── */}
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0, 1fr) 320px', gap: isMobile ? 20 : 28, alignItems: 'start' }}>

          {/* Mobile: card + stats first */}
          {isMobile && (
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ ...mc, delay: 0.05 }}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[{ label: 'Active plans', value: activeSubs.length, color: '#4ade80' }, { label: 'Friends referred', value: refCount, color: '#D4AF37' }].map(s => (
          ...
          {/* Subscriptions */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ ...mc, delay: isMobile ? 0.1 : 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              ...
              <Link href="/store" style={{ display: 'inline-flex', alignItems: 'center', height: 32, padding: '0 12px', borderRadius: 8, textDecoration: 'none', background: 'rgba(212,175,55,0.07)', border: '1px solid rgba(212,175,55,0.18)', color: '#D4AF37', fontSize: 12, fontWeight: 600 }}>
                + Add
              </Link>
            </div>

            {isLoadingSubs ? <Skeleton /> : subs.length === 0 ? (
              <div style={{ padding: '48px 20px', textAlign: 'center', background: 'rgba(255,255,255,0.02)', border: '1px dashed rgba(255,255,255,0.07)', borderRadius: 18 }}>
                <div style={{ fontSize: 28, color: '#222', marginBottom: 12 }}>◈</div>
              ...
              <motion.div layout style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[...activeSubs, ...expiredSubs].map((sub, i) => (
                  <motion.div key={sub.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * (isMobile ? 0.04 : 0.07) }}>
                    <SubCard sub={sub} isMobile={isMobile} />
                  </motion.div>
                ))}
              </motion.div>
            )}
          ...
          {/* Mobile bottom — refer + WA */}
          {isMobile && (
            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ ...mc, delay: 0.15 }}
              style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <AnimatePresence>
        {showEditProfile && (
          <EditProfileModal user={user} onClose={() => setShowEditProfile(false)} onSaved={(name, photo) => { setDisplayName(name); setPhotoURL(photo) }} />
        )}
      </AnimatePresence>
    </div>
  )
