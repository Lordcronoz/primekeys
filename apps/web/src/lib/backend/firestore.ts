...
export function getDb() {
  if (!_db) _db = getAdminApp().firestore()
  return _db
}

export function getAdminAuth() {
  if (!_auth) _auth = admin.auth(getAdminApp())
  return _auth
}

export const db        = new Proxy({} as admin.firestore.Firestore, { get: (_, p) => Reflect.get(getDb(), p) })
export const adminAuth = new Proxy({} as admin.auth.Auth, { get: (_, p) => Reflect.get(getAdminAuth(), p) })
...
export async function utrExists(utrNumber: string) {
  const snap = await db.collection('orders').where('utrNumber', '==', utrNumber).limit(1).get()
  return !snap.empty
}
...
export async function logPayment(data: {
  ...
  const ref = await db.collection('payments').add({ ...data, createdAt: admin.firestore.FieldValue.serverTimestamp() })
  return ref.id
}
...
export async function applyReferralReward(clientId: string) {
  await db.collection('clients').doc(clientId).update({ refCount: admin.firestore.FieldValue.increment(1) })
}
